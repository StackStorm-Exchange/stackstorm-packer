...
class FixActionTestCase(PackerBaseActionTestCase):
    __test__ = True
    action_cls = FixAction

    def test_init(self):
        action = self.get_action_instance({})
        self.assertIsInstance(action, FixAction)
    ...
    def test_run_success(self, mock_packer):
        ...
        test_dict = {'packerfile': 'test/file/packer',
                    'outputfile': 'test/out/file'}

        expected_result = "Expected Result"
        mock_packer.return_value.fix.return_value = expected_result

        result = action.run(**test_dict)
        self.assertEqual(result, expected_result)
        mock_packer.assert_called_once_with(test_dict['packerfile'])
        mock_packer.return_value.fix.assert_called_once_with(test_dict['outputfile'])
    ...
    def test_run_error(self, mock_packer):
        ...
        mock_packer.return_value.fix.side_effect = Exception("ERROR")
